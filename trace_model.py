...
class MeanActivation(nn.Module):
    def __init__(self):
        super(MeanActivation, self).__init__()

    def forward(self, x):
        return torch.clamp(torch.exp(x), min=1e-5, max=1e6)


class DispersionActivation(nn.Module):
    def __init__(self):
        super(DispersionActivation, self).__init__()

    def forward(self, x):
        return torch.clamp(F.softplus(x), min=1e-4, max=1e4)


class ZeroInflationActivation(nn.Module):
    def __init__(self):
        super(ZeroInflationActivation, self).__init__()
...
class TraceBottleneck(nn.Module):
    expansion = 4
    __constants__ = ['downsample']

    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None):
        super(TraceBottleneck, self).__init__()
        ...
        self.mdsab = MultiDilatedSpatialAttention(width)
...
class TraceAutoEncoder(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self._dec_mean = nn.Sequential(nn.Linear(784, 784), MeanActivation())        

        self._dec_disp = nn.Sequential(nn.Linear(784, 784), DispersionActivation())
        
        self._dec_pi = nn.Sequential(nn.Linear(784, 784), ZeroInflationActivation())
    ...
    def forward(self, x):
        ...
        return _mean, _disp, _pi, u, x


class MultiDilatedSpatialAttention(nn.Module):
    def __init__(self, channel, k_size=3, dilated_ratio=[2,4,6,8]):
        super(MultiDilatedSpatialAttention, self).__init__()
