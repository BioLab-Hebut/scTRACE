...
class HybridDeepClusterWithMetrics(nn.Module):
    def __init__(self, AE, data_loader, dataset_size, batch_size=100, 
        ...
        super(HybridDeepClusterWithMetrics, self).__init__()
        self.AE = AE
    ...
    def _unpack_batch_data(self, batch_data):
        if len(batch_data) == 4:
            norm_x, raw_x, target, index = batch_data
            return norm_x, raw_x, target, index, True  
    def model_evaluation(self):
        ...
        for batch_data in self.data_loader:
            norm_x, raw_x, target, index, has_raw_counts = self._unpack_batch_data(batch_data)
    ...
    def pretrain(self):
        ...
        has_raw_counts_globally = len(test_batch) == 4
        ...
        for T in range(self.pretraining_epoch):
            ...
            for batch_data in self.data_loader:
                ...
                if has_raw_counts:
                    raw_x_flat = raw_x.view(-1, 784)
                    nb_input = raw_x_flat
                
                nb_loss_val = torch.mean(
                    torch.sum(self.nb_loss(nb_input, _mean, _disp), dim=1)
                )
    
                nb_weight = 0.001  
                loss = mse_loss + nb_weight * nb_loss_val
                ...
                total_nb_loss += nb_loss_val.item()
        ...
        for T in range(self.MaxIter1):
            ...
            for batch_data in self.data_loader:
                ...
                for i in range(self.num_cluster):
                    ...
                    loss1 = self.clustering_cost(norm_x.view(-1, 784), y.view(-1, 784), u1, p[i, :].unsqueeze(0),
            ...
            NMI, ARI, ACC = self.model_evaluation()
            ARIlist.append(ARI)
            NMIlist.append(NMI)
            ACClist.append(ACC)
    ...
    def get_final_clustering_result(self):
        self.AE.eval()
        self.AE.cuda()
        self.u_mean = self.u_mean.cuda()
...
class HybridSCCAD:
    ...
    def hybrid_scCAD(self, csv_file_path=None, X_norm=None, cellNames=None, geneNames=None,
